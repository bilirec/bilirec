...
>
> 下表「内存峰值」为 **Docker 等容器监控里显示的内存用量**（更接近 bilirec 实际占用；长录且未开冷缓存释放时，监控可能因系统缓存录制文件而虚高）。改配置或版本后会有偏差。规划容器 **limit** 请仍用 [内存占用估算](https://www.bilirec.org/zh-cn/configuration/memory-estimation/)（偏保守）。
