...
type Config struct {
	...
	// DanmakuRecordEnabled gates whether the recorder starts danmaku (live chat)
	// recording alongside video recording. It only gates the recorder -> danmaku
	// service call; the bilibili danmaku protocol capability itself is always
	// available for other consumers (e.g. future websocket-based subcheck).
	DanmakuRecordEnabled bool
...
func provider(lc fx.Lifecycle) (*Config, error) {
	...
	c := &Config{
		...
		DanmakuRecordEnabled:               os.Getenv("DANMAKU_RECORD_ENABLED") == "true",
